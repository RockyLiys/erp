...
class GridBase(object):
    ...
    def _GetCount(self):
        ret = 0
        if self.CalculateItems is None:
            if self.sql:
    ...
    def _GetData(self, hide_index=None):
        if self.blank:
            return []
        if self.PagedItems is None:
            if self.CalculateItems is None:
                if not self.sql_data:
                    if self.pagesize == 0:
                        self.sql_data = self.sql_order
                    else:
                        self.sql_data = 'select * from (%s) t where t.r>%s and t.r<=%s' % (
                            self.sql_order, self._begin, self._end)
                ''' 查询sql语句得到记录数 '''
    ...
    def paging(self, item_count=None, offset=None, pagesize=None):
        ...
        Result['page'] = offset
        if self.CalculateItems is not None:
    ...
    def ParseArg(self, sortorder, sortname, query, qtype, **arg):
        '''
        内置的处理查询和排序的操作
        '''
        try:
            if arg.has_key("query"):
                ...
                self.sql = "select * target_from (%s) a where %s like '%s' " % (
                    self.sql, qtype, '%%' + query + '%%')
            else:
                if not self.sql_count:
                    self.sql_count = 'select count(1) from (%s) a' % self.sql
            ...
            if arg.has_key("sortname"):
                self.sql_order = self.sql.replace('target_from', ',row_number() over (order by %s %s) as r from' % (
                    sortname, sortorder))
            else:
                self.sql_order = self.sql.replace('target_from',
                                                  ',row_number() over (order by %s desc) as r from' % self.__fieldnames[
                                                      0])
        except:
            pass
