...
class DictNode(object):
    def __init__(self, values, encoding=None, root=None, section=[], orders=[], sectiondelimeter=section_delimeter,
        ...
        if self._items.has_key(name):
            ...
            if isinstance(value, dict):
                return DictNode(value, self._encoding, self._root, self._section + [name],
                                sectiondelimeter=self._section_delimeter, onelevel=self._onelevel, format=self._format)
            else:
                return value
        else:
            self._items[name] = {}
...
class DictIni(DictNode):
    def __init__(self, inifile=None, values=None, encoding=None, commentdelimeter='#',
                 sectiondelimeter=section_delimeter, onelevel=False, format="%s = %s", hideData=False, secretKey=None):
        self._items = {}
