...
class MsgType(CachingModel):
    ...
    class Admin(CachingModel.Admin):
        ...
        def initial_data():
            ...
            if not MsgType.objects.filter(msgtype_name=u"%(name)s" % {"name": _(u'考勤')},
                                          msgtype_value='8') and "mysite.att" in settings.INSTALLED_APPS:
                MsgType(id=2, msgtype_name=u"%(name)s" % {"name": _(u'考勤')}, msgtype_value='8', type='-1').save()
            if not MsgType.objects.filter(msgtype_name=u"%(name)s" % {"name": _(u'门禁')},
                                          msgtype_value='7') and "mysite.iaccess" in settings.INSTALLED_APPS:
            ...
            if not MsgType.objects.filter(msgtype_name=u"%(name)s" % {"name": _(u'消费')},
                                          msgtype_value='10') and "mysite.pos" in settings.INSTALLED_APPS:
                MsgType(id=5, msgtype_name=u"%(name)s" % {"name": _(u'消费')}, msgtype_value='10', type='-1').save()
            if not MsgType.objects.filter(msgtype_name=u"%(name)s" % {"name": _(u'会议')},
                                          msgtype_value='11') and "mysite.meeting" in settings.INSTALLED_APPS:
...
class GroupMsg(CachingModel):
    ...
    group = models.ForeignKey(Group, verbose_name=_(u'分配给角色'), limit_choices_to=~Q(**{'name': 'role_for_employee'}),
