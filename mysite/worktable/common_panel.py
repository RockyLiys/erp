...
from mysite.base.models import appOptions
...
from mysite.base.cached_model import cache_key
from mysite.base.backup import get_attsite_file
from mysite.base.dbapp.data_edit import form_for_model
...
def get_search_from(request, app_label, model_name):
    ...
    if model_name == "Device":
        device_disable_cols_dic = ['acpanel_type', 'device_use_type', 'dining.name', 'consume_model', 'com_port',
                                   'com_address', 'show_enabled|boolean_icon', 'show_fp_mthreshold', 'get_dstime_name',
...
def emp_type_change(dt, max_dt=None):
    ...
    for emp in qs:
        ...
        if len(msg.content) != 0:
            if (begin_date.year == emp.Hiredday.year) and (begin_date.month == emp.Hiredday.month) and (
                begin_date.day == emp.Hiredday.day):
    ...
    for emp in qs:
        ...
        if len(msg.content) != 0:
            if emp.Birthday.month == dt.month and emp.Birthday.day == dt.day:
                ...
                msg.content += str(dt.year) + "-" + str(emp.Birthday.month) + "-" + str(emp.Birthday.day) + u" %s" % _(
                    u"过生日！")
            msg.monitor_last_time = datetime.datetime.now()
